module Mod_plcd_NeoHookeanUpdatedFormulation
   use typre
   use Mod_plcd_Material
   use Mod_Listen
   use Mod_Element
   use Mod_plcd_ConstitutiveTensor
   implicit none
   private
   public NeoHookeanUpdatedMaterial, NeoHookeanUpdatedMaterial_ED

   type, extends(PLCDMaterial) :: NeoHookeanUpdatedMaterial
   
      real(rp) :: lambda, mu
      class(ConstitutiveTensor), pointer :: auxCT => null()

contains
      procedure :: SpecificCreateElementData => NH_SpecificCreateElementData
      procedure :: SpecificReadData => NH_SpecificReadData
      procedure :: SpecificScatterData => NH_SpecificScatterData
      procedure :: SpecificSetup       => NH_SpecificSetup
      procedure :: IsElementSizeRequiredByEMDs => NH_IsElementSizeRequiredByEMDs

   end type
   
   type :: NeoHookeanUpdatedMaterial_GPD
      real(rp), allocatable :: C(:,:) 
      real(rp), allocatable :: S(:) 
      real(rp), allocatable :: F_mat(:,:), F_spat(:,:)
      real(rp) :: J 
      
   end type

   type, extends(ElementMaterialData) :: NeoHookeanUpdatedMaterial_ED

      class(NeoHookeanUpdatedMaterial), pointer :: Material => NULL()
      type(NeoHookeanUpdatedMaterial_GPD), allocatable :: GP(:)

contains

      procedure :: ComputeHistoryAndConstitutiveTensor => NH_ComputeHistoryAndConstitutiveTensor
      procedure :: ComputeStress => NH_ComputeStress
      procedure :: GetConstitutiveTensorPointer => NH_GetConstitutiveTensorPointer
      procedure :: GetStressTensorPointer => NH_GetStressTensorPointer
      procedure :: GetMaterialPointer

      !Adaptivity
      procedure :: GetHistoryVariablesNdofn
      procedure :: Finalize
      procedure :: CopyTo
      procedure :: SpecificHistoryAddFromGaussArray
      procedure :: SpecificHistoryGetToGaussArray
      procedure :: SpecificHistoryFinalRefinementUpdate

      procedure :: GetBufferSize
      procedure :: ToBuffer
      procedure :: InitializeFromBuffer

      !For u-p elements
      procedure :: GetInverseVolumetricDeformationModulus
      procedure :: GetSecantCNorm
      procedure :: SetPressureForComputeHistoryAndConstitutiveTensor


   end type
   
      

contains

   subroutine NH_SpecificSetup(a)
      implicit none
      class(NeoHookeanUpdatedMaterial) :: a
      
      allocate(a%auxCT,source=a%CT)
   end subroutine   

   subroutine NH_SpecificCreateElementData(a,pgaus,ElementData)
      implicit none
      class(NeoHookeanUpdatedMaterial), target :: a
      integer(ip) :: pgaus
      class(ElementMaterialData), pointer :: ElementData

      type(NeoHookeanUpdatedMaterial_ED), pointer :: NH_ElementData
      type(NeoHookeanUpdatedMaterial_GPD), pointer :: GP(:)
      integer(ip) :: igaus

      allocate(NH_ElementData)
      NH_ElementData%Material => a
      
      allocate(NH_ElementData%GP(pgaus))
      GP => NH_ElementData%GP
      
      do igaus = 1,pgaus
         allocate(GP(igaus)%C(a%CT%VoigtSize,a%CT%VoigtSize))
         allocate(GP(igaus)%S(a%CT%VoigtSize))
         allocate(GP(igaus)%F_mat(a%ndime,a%ndime))
         allocate(GP(igaus)%F_spat(a%ndime,a%ndime))
         GP(igaus)%J=0.0_rp
      enddo
      

      ElementData => NH_ElementData
      
   end subroutine


   subroutine NH_SpecificReadData(a,Listener)
      class(NeoHookeanUpdatedMaterial) :: a
      type(ListenFile) :: Listener
      
   end subroutine

   subroutine NH_SpecificScatterData(a)
      use MPI
      class(NeoHookeanUpdatedMaterial) :: a
      
      call ComputeLameParameters(a)
      
   end subroutine

   subroutine NULLSUB(a)
      class(NeoHookeanUpdatedMaterial):: a
   end subroutine

   subroutine NH_IsElementSizeRequiredByEMDs(a,isRequired)
      implicit none
      class(NeoHookeanUpdatedMaterial), target :: a
      logical :: IsRequired

      isRequired = .false.
   end subroutine
   
   subroutine ComputeLameParameters(a)
      implicit none
      class(NeoHookeanUpdatedMaterial) :: a
      real(rp) :: E,nu
      
      call a%CT%GetYoungModulus(E)
      call a%CT%GetPoissonRatio(nu)
      
      a%mu     = E/(2.0_rp + 2.0_rp*nu)
      a%lambda = nu*E/((1.0_rp + nu)*(1.0_rp - 2.0_rp*nu)) 
      
   end subroutine


   !-------------------------------------------------------------------
   !EMDS

   subroutine NH_ComputeHistoryAndConstitutiveTensor(a,igaus,gradDisp)
      class(NeoHookeanUpdatedMaterial_ED), target :: a
      integer(ip) :: igaus
      real(rp) :: gradDisp(:,:)
      
      call ComputeDeformationGradientandJ(a,igaus,gradDisp)
      call NH_ComputeConstitutiveTensor(a, igaus)
   end subroutine

   subroutine NH_ComputeConstitutiveTensor(a, igaus)
      class(NeoHookeanUpdatedMaterial_ED), target :: a
      real(rp) :: lambda, mu, E, nu
      integer(ip) :: igaus
      real(rp), pointer :: J => NULL()
      real(rp), pointer :: C(:,:) => NULL()
      
      call GetJPointer(a,igaus,J)
      
      lambda = a%Material%lambda
      mu  = a%Material%mu -lambda*log(J) 
      
      E = mu*(3*lambda+2*mu)/(lambda+mu)
      nu = lambda/(2*(lambda+mu))
      
      call a%Material%auxCT%SetYoungModulus(E)
      call a%Material%auxCT%SetPoissonRatio(nu)
      
      call a%Material%auxCT%Build
      
      call a%Material%CT%GetPointerToC(C)
      a%GP(igaus)%C = C

      a%GP(igaus)%C =  (1.0_rp/J)*a%GP(igaus)%C

   end subroutine
   
   subroutine ComputeDeformationGradientandJ(a,igaus,gradDisp)
      class(NeoHookeanUpdatedMaterial_ED), target :: a
      integer(ip) :: idime, igaus
      integer(ip), pointer :: ndime
      real(rp) :: gradDisp(:,:)
      
      type(NeoHookeanUpdatedMaterial_GPD), pointer :: GP => NULL()
      
      ndime => a%Material%ndime
         
      GP => a%GP(igaus)

      !we transform gradDisp into deformation tensor F
      !u = x_sp - X_mat; grad(u) = ii - F(x)^-1
      GP%F_spat= - gradDisp
      forall(idime = 1:ndime) GP%F_spat(idime,idime) = GP%F_spat(idime,idime) + 1.0_rp 
      
      !Compute F_mat as the inverse of F_spat and J as determinant
      
      call invmtx(GP%F_spat,GP%F_mat,GP%J,ndime)

      GP%J= 1.0_rp/GP%J
      
      if(GP%J .lt. epsilon(0.0_rp) ) then
         call runend('PLCD Base Elmope: Negative element determinant')
      end if
      
   end subroutine
   
   subroutine NH_ComputeStress(a,igaus,gradDisp)
      class(NeoHookeanUpdatedMaterial_ED), target :: a
      integer(ip) :: igaus, idime
      integer(ip), pointer :: ndime
      real(rp) :: gradDisp(:,:)
      real(rp), pointer :: F_mat(:,:) => NULL()
      real(rp), pointer :: F_spat(:,:) => NULL()
      real(rp), pointer :: J, lambda, mu
      !real(rp) :: Identity(a%Material%ndime,a%Material%ndime)
      real(rp) :: CauchyStress(a%Material%ndime,a%Material%ndime)
      real(rp) :: LeftCauchyTensor(a%Material%ndime,a%Material%ndime)

      call GetDeformationGradientPointer(a,igaus,F_mat,F_spat) 
      call GetJPointer(a,igaus,J)
      call GetLameParametersPointer(a,lambda,mu)
      
      ndime => a%Material%ndime

      LeftCauchyTensor = 0.0_rp
      
      CauchyStress = 0.0_rp
       
      !Calculate Left Cauchy Green Deformation tensor
      LeftCauchyTensor = matmul(F_mat,transpose(F_mat))
 
      !Calculate Kirchhoff stress and solve to obtain Cauchy => tau = J * sigma
      !CauchyStress = (1.0_rp/J)*(lambda*log(J)*Identity + mu*(LeftCauchyTensor-Identity))
      CauchyStress = mu*LeftCauchyTensor
      forall(idime = 1:ndime) CauchyStress(idime,idime) = CauchyStress(idime,idime) + lambda*log(J)-mu
      CauchyStress = (1.0_rp/J)*CauchyStress
       
      !Get into voight notation
      call a%Material%CT%GetStressVector(CauchyStress,a%GP(igaus)%S(1:a%Material%CT%VoigtSize))

   end subroutine

   subroutine NH_GetStressTensorPointer(a,igaus,S)
      class(NeoHookeanUpdatedMaterial_ED), target:: a
      integer(ip) :: igaus
      real(rp), pointer :: S(:)

      S => a%GP(igaus)%S(1:a%Material%CT%VoigtSize)
   end subroutine

   subroutine NH_GetConstitutiveTensorPointer(a,igaus,C)
      class(NeoHookeanUpdatedMaterial_ED), target:: a
      integer(ip) :: igaus
      real(rp), pointer :: C(:,:)

      C => a%GP(igaus)%C
   end subroutine

   subroutine GetMaterialPointer(a,Material)
      class(NeoHookeanUpdatedMaterial_ED), target :: a
      class(PLCDMaterial), pointer :: Material

      Material => a%Material
   end subroutine
   
   subroutine GetJPointer(a,igaus,J)
      class(NeoHookeanUpdatedMaterial_ED), target:: a
      integer(ip) :: igaus
      real(rp), pointer :: J

      J => a%GP(igaus)%J
   end subroutine
   
   subroutine GetDeformationGradientPointer(a,igaus,F_mat,F_spat)
      class(NeoHookeanUpdatedMaterial_ED), target:: a
      integer(ip) :: igaus
      real(rp), pointer :: F_mat(:,:), F_spat(:,:)

      F_mat => a%GP(igaus)%F_mat
      F_spat => a%GP(igaus)%F_spat
   end subroutine
   
   subroutine GetLameParametersPointer(a,lambda,mu)
      class(NeoHookeanUpdatedMaterial_ED), target:: a
      real(rp), pointer :: lambda, mu

      lambda => a%Material%lambda
      mu => a%Material%mu
   end subroutine
   

   !Adaptivity
   subroutine GetHistoryVariablesNdofn(a,ndofn)
      class(NeoHookeanUpdatedMaterial_ED) :: a
      integer(ip) :: ndofn

      ndofn = 0
   end subroutine

   subroutine SpecificHistoryGetToGaussArray(a,GaussArray)
      class(NeoHookeanUpdatedMaterial_ED) :: a
      real(rp) :: GaussArray(:,:)


   end subroutine

   subroutine SpecificHistoryAddFromGaussArray(a,GaussArray)
      class(NeoHookeanUpdatedMaterial_ED) :: a
      real(rp) :: GaussArray(:,:)


   end subroutine

   subroutine CopyTo(a,EMD)
      class(NeoHookeanUpdatedMaterial_ED) :: a
      class(ElementMaterialData), pointer :: EMD

   end subroutine

   subroutine Finalize(a)
      class(NeoHookeanUpdatedMaterial_ED) :: a
      integer(ip) :: igaus

      do igaus = 1,size(a%GP,1)
         deallocate(a%GP(igaus)%S)
         deallocate(a%GP(igaus)%C)
         deallocate(a%GP(igaus)%F_mat)
         deallocate(a%GP(igaus)%F_spat)
      enddo
      deallocate(a%GP)
      
   end subroutine

   subroutine GetBufferSize(a,BufferSize)
      class(NeoHookeanUpdatedMaterial_ED) :: a
      integer(ip) :: BufferSize

      BufferSize = 0
   end subroutine

   subroutine ToBuffer(a,Buffer)
      class(NeoHookeanUpdatedMaterial_ED) :: a
      real(rp) :: Buffer(*)
   end subroutine

   subroutine InitializeFromBuffer(a,Buffer)
      class(NeoHookeanUpdatedMaterial_ED) :: a
      real(rp) :: Buffer(*)
   end subroutine

   !For u-p elements
   subroutine GetInverseVolumetricDeformationModulus(a,igaus,invK)
      class(NeoHookeanUpdatedMaterial_ED) :: a
      integer(ip) :: igaus
      real(rp) :: invK

      call a%Material%CT%GetInverseVolumetricDeformationModulus(invK)
   end subroutine

   subroutine GetSecantCNorm(a,igaus,SecantCNorm)
      class(NeoHookeanUpdatedMaterial_ED) :: a
      real(rp) :: SecantCNorm
      integer(ip) :: igaus

      call a%Material%CT%GetCNorm(SecantCNorm)

   end subroutine

   subroutine SetPressureForComputeHistoryAndConstitutiveTensor(a,Pressure)
      class(NeoHookeanUpdatedMaterial_ED) :: a
      real(rp) :: Pressure

      !The pressure is not needed for elastic materials

   end subroutine

    subroutine SpecificHistoryFinalRefinementUpdate(a)
      class(NeoHookeanUpdatedMaterial_ED) :: a



   end subroutine

end module