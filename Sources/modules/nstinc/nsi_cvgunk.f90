subroutine nsi_cvgunk(a,itask)
   ...
   real(rp)          :: vensi,prnsi,rinsi,rivel,ripre
   integer           :: ierr
   logical           :: isALE
   ...
   case(1)
      ...
      call a%InnerResiduals(vensi,prnsi)
      
      !If wall law update statistiscs
      call nsi_BouwalStats(a,1)
      ...
      if (a%MPIrank == a%MPIroot) then
         ...
         write(a%lun_conve,101) a%istep,a%itera,a%ctime,vensi,prnsi
   ...
   case(4)
      ...
      call a%Mesh%GetNpoinLocal(npoinLocal)
      call a%Mesh%GetNdime(ndime)
      call vecresMPI(two,ndime*npoinLocal,a%veloc(:,:,1),a%veloc_cp(:,:),rivel,zensi,a%kfl_MPIComType,a%MPIcomm,a%MPIroot,a%MPIrank,a%MPIsize)
      call vecresMPI(two,npoinLocal,      a%press(:,:),  a%press_cp(:),  ripre,zensi,a%kfl_MPIComType,a%MPIcomm,a%MPIroot,a%MPIrank,a%MPIsize)

      a%cpres = max(rivel,ripre)
      call MPI_ALLREDUCE(MPI_IN_PLACE,a%cpres,1,MPI_REAL8,MPI_MAX,a%MPIcomm,ierr)
      call MPI_ALLREDUCE(MPI_IN_PLACE,rivel,1,MPI_REAL8,MPI_MAX,a%MPIcomm,ierr)
      call MPI_ALLREDUCE(MPI_IN_PLACE,ripre,1,MPI_REAL8,MPI_MAX,a%MPIcomm,ierr)
      ...
      if (a%MPIrank == a%MPIroot) then
          ...
          write(a%lun_cpconve,101) a%istep,a%cpiter,a%ctime,rivel,ripre,a%relax 
          if (a%kfl_flush == 1) then
...
subroutine nsi_InnerResiduals(a,rinsi,rprnsi)
   ...
   call a%Mesh%GetNpoinLocal(npoinLocal)
   call a%Mesh%GetNdime(ndime)
   call vecresMPIHeterogeneous(2_ip,ndime+1,ndime,npoinLocal,1,1,ndime,a%unkno,a%veloc(1,1,1),rinsi,zensi,a%MPIcomm,a%MPIroot)
   call vecresMPIHeterogeneous(2_ip,ndime+1,1,npoinLocal,ndime+1,1,1,a%unkno,a%press(1,1),rprnsi,zensi,a%MPIcomm,a%MPIroot)     
