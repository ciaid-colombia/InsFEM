subroutine supf_cvgunkU(a,itask)
   ...
   interface
      ...
      subroutine supf_InnerResidualsU(a,rinsi,prnsi)
         use typre      
   ...
   case(1)
      ...
      call supf_InnerResidualsU(a,rinsi,prnsi)
         
      if (a%MPIrank == a%MPIroot) then
         if(((rinsi<a%cotol).or.(a%itera>=a%maxit))) a%kfl_goite = 0       

         !Some Postprocessing, only root
         if(ipass==0) then
            ipass=1
         ...
         write(a%lun_conve,101) a%istep,a%itera,a%ctime,rinsi,prnsi
