  module Mod_sldsup_ComputeResidualProjection
   use typre
...
contains
   ...
   subroutine resProAllocate
      ...
      call a%Memor%alloc(a%ndofn,npoin,wrepro,'wrepro','sldsup_EndElmope')
   end subroutine
   
   subroutine AllocRep
      implicit none
      call a%Memor%alloc(a%ndofn,e%mnode,elres,'elres','sldsup_EnditeElmope')
   end subroutine
   
   subroutine DeallocRep
      implicit none
      call a%Memor%dealloc(a%ndofn,e%mnode,elres,'elres','sldsup_EnditeElmope')
   end subroutine
   ...
   subroutine ProjectResidual
      ...
      call sup%Project(a%ndofn,wrepro) 
      
      sup%repro = wrepro
      call move_alloc(wrepro,sup%repro)
      
      !Dealloc wrepro
      call a%Mesh%GetNpoin(npoin)
      call a%Memor%deallocObj(0,'wrepro','sldsup_EndElmope',(a%ndofn)*npoin*rp)
