module Mod_nsc_pr_ComputeSubscales
   use typre
...
contains
      ...
      case(1)
         ...
         if (kfl_IsSet == -1) then
            ...
            if (a%kfl_tacsg == 0) then
               ...
               call ConcatenateProcedures(ProcHook_nsc_pr_InGaussElmats,ComputeSubgridScaleQSS)
            ...
            elseif ( a%kfl_tacsg == 1) then
               ...
               call ConcatenateProcedures(ProcHook_nsc_pr_InGaussElmats,ComputeSubgridScaleDSS)
      ...
      case(1)
         ...
         if (kfl_IsSetGetSubscales == -1) then
            ...
            if (a%kfl_trasg == 0) then
               ...
               call ConcatenateProcedures(ProcHook_nsc_pr_InGaussElmats,GetSubgridScale)
            endif
         endif  
      case(100)
   ...
   subroutine ComputeSubgridScaleDSS
      ...
      a%mosgs(ielem)%a(1:e%ndime,1,e%igaus) = timom(2)*(gpmSGSpaceResidual(1:e%ndime) + SGSrhm(1:e%ndime)*ReferenceDtinv)
      a%cosgs(ielem)%a(1,e%igaus) = timom(1)*(gpdSGSpaceResidual(1) + SGSrhd*ReferenceDtinv)
      a%ensgs(ielem)%a(1,e%igaus) = timom(3)*(gpeSGSpaceResidual(1) + SGSrhe*ReferenceDtinv)
