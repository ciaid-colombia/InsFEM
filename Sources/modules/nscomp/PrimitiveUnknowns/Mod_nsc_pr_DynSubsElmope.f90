module Mod_nsc_pr_DynSubsElmope
   use typre
   use Mod_nsc_pr_BaseElmope
   implicit none
   private
   public SetPointersDynSubsElmope,nsc_pr_elmrhd_dss,nsc_pr_elmrhm_dss,nsc_pr_elmrhe_dss
...
contains
      ...
      case(1)
         if (kfl_IsSet == -1) then
            ...
            if (a%kfl_tacsg == 1) then
               ...
               call ConcatenateProcedures(ProcHook_nsc_pr_Initializations,AllocDSS)
               call ConcatenateProcedures(ProcHook_nsc_pr_Finalizations, DeallocDSS)
               
               if (a%kfl_repro == 1) then
                  !This adds the subscale at the previous time step to the RHS
                  call ConcatenateProcedures(ProcHook_nsc_pr_InGaussElmats,InGaussElmatsDSS_OSS)
               
               elseif (a%kfl_repro == 0) then
                  !If the subscales are not orthogonal, we also need to add the term
                  !(v,\partial_t \tilde(u))
                  !See the documentation folder
                  call ConcatenateProcedures(ProcHook_nsc_pr_ComputeTestf,DynamicSgsTestf_ASGS)
                  
                  !This adds the terms involving the subscale at the previous time step to the RHS
                  call ConcatenateProcedures(ProcHook_nsc_pr_InGaussElmats,InGaussElmatsDSS_ASGS)
               else
   ...
   subroutine AllocDSS
      ...
      call a%Memor%alloc(e%ndime,SGSrhm,'SGSrhm','nsc_pr_elmope')
      call a%Memor%alloc(e%mnode,LTdd_static,'LTdd_static','nsc_pr_elmope')
      call a%Memor%alloc(e%ndime,e%ndime,e%mnode,LTmm_static,'LTmm_static','nsc_pr_elmope')
      call a%Memor%alloc(e%mnode,LTee_static,'LTee_static','nsc_pr_elmope')
   ...
   subroutine DeAllocDSS
      ...
      call a%Memor%dealloc(e%ndime,SGSrhm,'SGSrhm','nsc_pr_elmope')
      call a%Memor%dealloc(e%mnode,LTdd_static,'LTdd_static','nsc_pr_elmope')
      call a%Memor%dealloc(e%ndime,e%ndime,e%mnode,LTmm_static,'LTmm_static','nsc_pr_elmope')
      call a%Memor%dealloc(e%mnode,LTee_static,'LTee_static','nsc_pr_elmope')
   end subroutine
   ...
   subroutine InGaussElmatsDss_OSS
      ...
      !Compute RHS contribution of transient subscales
      call ComputeRHSContribution(e,a%cosgs(ielem)%a(2,e%igaus),a%mosgs(ielem)%a(1:e%ndime,2,e%igaus),a%ensgs(ielem)%a(2,e%igaus),0.0_rp,auxarray,0.0_rp,SGSrhd,SGSrhm,SGSrhe)

      !Compute contributions to RHS : Continuity
       call nsc_pr_elmrhd_dss(e,dvol,ReferenceDtinv,SGSrhd,SGSrhm,SGSrhe,LTdd,LTdm,LTde,elrhd)
            
      !Compute contributions to RHS : Momentum
       call nsc_pr_elmrhm_dss(e,dvol,ReferenceDtinv,SGSrhd,SGSrhm,SGSrhe,LTmd,LTmm,LTme,elrhm)

      !Compute contributions to RHS : Energy
       call nsc_pr_elmrhe_dss(e,dvol,ReferenceDtinv,SGSrhd,SGSrhm,SGSrhe,LTed,LTem,LTee,elrhe)
   ...
   subroutine InGaussElmatsDss_ASGS
      ...
      !Compute contributions to RHS : Continuity
       call nsc_pr_elmrhd_dss(e,dvol,ReferenceDtinv,SGSrhd,SGSrhm,SGSrhe,LTdd_static,LTdm,LTde,elrhd)
            
      !Compute contributions to RHS : Momentum
       call nsc_pr_elmrhm_dss(e,dvol,ReferenceDtinv,SGSrhd,SGSrhm,SGSrhe,LTmd,LTmm_static,LTme,elrhm)

      !Compute contributions to RHS : Energy
       call nsc_pr_elmrhe_dss(e,dvol,ReferenceDtinv,SGSrhd,SGSrhm,SGSrhe,LTed,LTem,LTee_static,elrhe)
   ...
   subroutine nsc_pr_elmrhd_dss(e,dvolu,dtinv,SGSrhd,SGSrhm,SGSrhe,LTdd,LTdm,LTde,elrhd)
      !-----------------------------------------------------------------------
      !
   ...
   subroutine nsc_pr_elmrhm_dss(e,dvolu,dtinv,SGSrhd,SGSrhm,SGSrhe,LTmd,LTmm,LTme,elrhm)
      !-----------------------------------------------------------------------
      !
   ...
   subroutine nsc_pr_elmrhe_dss(e,dvolu,dtinv,SGSrhd,SGSrhm,SGSrhe,LTed,LTem,LTee,elrhe)
      !-----------------------------------------------------------------------
      !
