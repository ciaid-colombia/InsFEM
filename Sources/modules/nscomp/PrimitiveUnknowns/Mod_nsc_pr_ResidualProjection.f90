  module Mod_nsc_pr_ComputeResidualProjection
   use typre
...
contains
   ...
   subroutine SetPointersComputeResidualProjection(itask)
      ...
      case(1)
         if (kfl_IsSet == -1) then
            ...
            call ConcatenateProcedures(ProcHook_nsc_pr_Initializations,AllocateRep)
            call ConcatenateProcedures(ProcHook_nsc_pr_Finalizations,DeallocateRep)
         
            call ConcatenateProcedures(ProcHook_nsc_pr_ElmatsToZero,RepElmatsToZero)
            ...
            call ConcatenateProcedures(ProcHook_nsc_pr_InGaussElmatsAssembly,GpresToElres)
            
            call ConcatenateProcedures(ProcHook_nsc_pr_AssemblyEndite,AssemblyResidual)
            call ConcatenateProcedures(ProcHook_nsc_pr_PostLoop,ProjectResidual)
         endif  
      case(100)
   ...
   subroutine AllocateRep
      ...
      call a%Memor%alloc(a%ndofn,npoin,wrepro,'wrepro','nsc_pr_EndElmope')
      call a%Memor%alloc(a%ndofn,e%mnode,elres,'elres','nsc_pr_EndElmope')
   ...
   subroutine DeallocateRep
      implicit none
      call a%Memor%dealloc(a%ndofn,e%mnode,elres,'elres','nsc_pr_EndElmope')
   end subroutine
   ...
   subroutine ProjectResidual
      ...
      call a%Mesh%GetNdime(ndime)
      call a%Mesh%GetNpoin(npoin)
      call a%Memor%deallocObj(0,'wrepro','nsc_pr_EndElmope',a%ndofn*npoin*rp)
