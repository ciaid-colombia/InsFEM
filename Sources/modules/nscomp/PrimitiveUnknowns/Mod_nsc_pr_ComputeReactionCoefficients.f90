module Mod_nsc_pr_ComputeReactionCoefficients
   use typre
...
contains
   ...
   subroutine SetPointersReactionCoefficients(itask)
      ...
      case(1)
         ...
         if (kfl_IsSet == -1) then
            ...
            call ConcatenateProcedures(ProcHook_nsc_pr_Initializations,AllocateReactionCoefficients)
            call ConcatenateProcedures(ProcHook_nsc_pr_Finalizations,DeallocateReactionCoefficients)
            call ConcatenateProcedures(ProcPointer_nsc_pr_ComputeTransportCoefficients,ReactionCoefficientsToZero)
            !Sources Exists
            call ConcatenateProcedures(ProcPointer_nsc_pr_ComputeTransportCoefficients,ComputeSourcesCoefficients)
            !Rectangular Damping Exists
            if (a%ndamp > 0) then
               kfl_damp = 1
               call ConcatenateProcedures(ProcPointer_nsc_pr_ComputeTransportCoefficients,ComputeRectangularDamping)
            endif

            !Radial Damping Exists
            if (a%rdamp > 0) then
               kfl_damp = 1
               call ConcatenateProcedures(ProcPointer_nsc_pr_ComputeTransportCoefficients,ComputeRadialDamping)
            endif

            if (kfl_damp == 1) then
                call ConcatenateProcedures(ProcPointer_nsc_pr_ComputeTransportCoefficients,ComputeDampingCoefficients)
            endif
   ...
   subroutine ComputeSourcesCoefficients
     ...
     integer(ip) :: idime
      ...
      do idime=1,e%ndime
         Sem(idime,:) = Sem(idime,:) + gpden*a%gravi(idime)*a%grnor*e%shape(:,e%igaus)!f
      end do
   ...
   subroutine ComputeDampingCoefficients
     ...
     !Mass equation
     Sdd(:) = Sdd(:) - gpden*dampc*e%shape(:,e%igaus)*LHSdtinv/gpadp
     !Momentum equation
     do idime=1,e%ndime
        Smm(idime,idime,:) = Smm(idime,idime,:) - gpden*dampc*e%shape(:,e%igaus)*LHSdtinv
     end do
     !Energy equation
     See(:) = See(:) - gpden*accph*dampc*e%shape(:,e%igaus)*LHSdtinv 
