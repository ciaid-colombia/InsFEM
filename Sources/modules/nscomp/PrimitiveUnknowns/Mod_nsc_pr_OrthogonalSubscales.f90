module Mod_nsc_pr_OrthogonalSubscales
   use typre
...
contains
         ...
         if (kfl_IsSet == -1) then
            ...
            if (a%kfl_repro /= 0) then
               ...
               if (a%kfl_repro == 1) then
                  !Matrices
                  call ConcatenateProcedures(ProcHook_nsc_pr_InGaussElmats,InGaussElmatsRep)
               endif
            endif
  ...
  !For ResidualProjection
   subroutine InGaussElmatsRep
      ...
      call nsc_pr_elmrhd_oss(e,dvol,gprep,LTdd,LTdm,LTde,elrhd)     
      
      !Compute contributions to RHS : Block velocity
      call nsc_pr_elmrhm_oss(e,dvol,gprep,LTmd,LTmm,LTme,elrhm)
      
      !Compute contributions to RHS : Block temperature
      call nsc_pr_elmrhe_oss(e,dvol,gprep,LTed,LTem,LTee,elrhe)     

   end subroutine
   
   subroutine nsc_pr_elmrhd_oss(e,dvolu,gprep,LTdd,LTdm,LTde,elrhd)
      !-----------------------------------------------------------------------
      !
   ...
   end subroutine nsc_pr_elmrhd_oss

   subroutine nsc_pr_elmrhm_oss(e,dvolu,gprep,LTmd,LTmm,LTme,elrhm)
      !-----------------------------------------------------------------------
      !
   ...
   end subroutine nsc_pr_elmrhm_oss

   subroutine nsc_pr_elmrhe_oss(e,dvolu,gprep,LTed,LTem,LTee,elrhe)
      !-----------------------------------------------------------------------
      !
   ...
   end subroutine nsc_pr_elmrhe_oss
