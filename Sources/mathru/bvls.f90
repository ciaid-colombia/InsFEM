...
INTEGER, INTENT(IN)        :: m
INTEGER, INTENT(IN)        :: n
REAL (rp), INTENT(IN OUT)  :: a(m,n)
REAL (rp), INTENT(IN OUT)  :: b(m)
REAL (rp), INTENT(IN)      :: bl(n)
REAL (rp), INTENT(IN)      :: bu(n)
...
REAL (rp), PARAMETER  :: eps = 1.0E-11_rp
INTEGER               :: i, iact, ifrom5, it, j, jj, k, k1, kk, ks, mm, mm1, &
                         nact, nbound, noldb
REAL (rp)             :: act(m,m+2), alf, alpha, bad, bdiff, bnorm, bound, &
...
!   test.
  IF (resq < 0.0_rp .OR. (ifrom5 > 0 .AND. zz(nact) > bu(iact)) .OR.   &
...
!  Steps 8, 9.
  ...
  DO  k = k1, nact
    ...
    IF (alf < alpha) THEN
      alpha = alf
      jj = j
      sj = SIGN(1.0_rp, zz(nact+1-k)-bl(j))
    END IF
  END DO
...
!   (Steps 6-11) terminates after at most  nact  steps.
  ...
  DO  k = 1, nact
    j = istate(k+noldb)
    IF (bu(j)-x(j) <= 0.0_rp .OR. (j == jj .AND. sj > 0.0_rp)) THEN
...
!  Move x(j) to its upper bound.
    ...
    ELSE IF (x(j)-bl(j) <= 0.0_rp .OR. (j == jj .AND. sj < 0.0_rp)) THEN
...
REAL (rp) :: const, total, dot, qv1, sq, u1
INTEGER   :: i, j, j1, jj
