...
sub new {
  ...
  # add it to ParallelForkManager object so children can use it
...
sub _waitpid_non_blocking {
    ...
    for my $pid ( $self->running_procs ) {
        ...
        warn "child process '$pid' disappeared. A call to `waitpid` outside of ParallelForkManager might have reaped it.\n";
...
ParallelForkManager - A simple parallel processing fork manager
...
=head1 SYNOPSIS
  ...
  use ParallelForkManager;

  my $pm = ParallelForkManager->new($MAX_PROCESSES);
...
The code for a downloader would look something like this:
  ...
  my $pm = ParallelForkManager->new(30);
...
you must instantiate another ParallelForkManager object!
...
Instantiate a new ParallelForkManager object. You must specify the maximum
...
C<ParallelForkManager>. Note that children are still reported as running
...
When it comes to waiting for child processes to terminate, C<ParallelForkManager> is between 
...
the safest way to ensure that C<ParallelForkManager> won't interfere with 
...
Alternatively, C<ParallelForkManager> can call C<waitpid> such that it will
...
I<waitpid_blocking_sleep> period to C<0>, which will enable C<waitpid> call blocking.
    ...
    my $pm = ParallelForkManager->new( 4 );
...
This small example can be used to get URLs in parallel.
  ...
  my $pm = ParallelForkManager->new(10);
...
Example of a program using callbacks to get child exit codes:
  ...
  my $pm = ParallelForkManager->new($max_procs);
...
In this simple example, each child sends back a string reference.
  ...
  use ParallelForkManager 0.7.6;
  use strict;

  my $pm = ParallelForkManager->new(2, '/server/path/to/temp/dir/');
...
=for example begin
  ...
  my $pm = ParallelForkManager->new(20);  # using the system temp dir $L<File::Temp::tempdir()
...
ParallelForkManager uses temporary files when 
...
the directory used by ParallelForkManager is restricted to the current user
only (the default behavior is to create a directory,
via L<File::Temp>'s C<tempdir>, which does that).

=head1 TROUBLESHOOTING

=head2 PerlIO::gzip and ParallelForkManager do not play nice together
...
Do not use ParallelForkManager in an environment, where other child
processes can affect the run of the main program, so using this module
is not recommended in an environment where fork() / wait() is already used.

If you want to use more than one copies of the ParallelForkManager, then
you have to make sure that all children processes are terminated, before you
use the second object in the main program.

You are free to use a new copy of ParallelForkManager in the child
...
This software is copyright (c) 2000 by BalÃ¡zs SzabÃ³.
